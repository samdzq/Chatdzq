                            ...
                            <li class="d-flex message">
                                ...
                                <div class="message-body">
                                    ...
                                    <div class="message-row d-flex align-items-center" v-for="(n_t,n_index) in normal_message" :key="n_index">
                                        <div v-if="n_t.info" class="message-content p-3" v-html="n_t.info"></div>
                                    </div>
...
const get_normal_tips = () => {
    get_normal().then((res: any) => {
        const data = res._rawValue.data
        data[0].info =  data[0].info.replace('Chat Aix','ChatDZQ')
        data[1].info = ''
        normal_message.value = data
...
const  submitForm = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid, fields) => {
        ...
        if (valid) {
            ...
            const res = await fetch(`https://mg.openaix.top/api/send_bot`, {
                method: 'POST',
                headers: headers,
                body: JSON.stringify({
...
const  submitFormNoLogin = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid, fields) => {
        ...
        if (valid) {
            ...
            const res = await fetch(`https://mg.openaix.top/api/no_send_bot`, {
                method: 'POST',
                headers: headers,
                body: JSON.stringify({
                    info: ruleForm.message_input,
